...
\blist
    \item $\closure\ \tsig \longto \epsilon\ \varnothing\ \bigl(\gen{\ell,\xi,s},u\mapsto\epsilon,\xi\},s+[\ell\mapsto u]\bigr)$ where $\ell\mapsto u$ means that if $\ell=(x)$ then $x\mapsto u$.
        Otherwise $\ell=(x_1,\dots,x_n)$ and $u=(u_1,\dots,u_n)$ and $x_i\mapsto u_i$ (recursively).
\elist
