...
We add two abstract types $\op$ and $\value$ with initial reduction rules:
\blist
    \item $\value\ \op\ s \longto \op\ \snd\ \bigl(v,f\to(v,f),\epsilon,s\bigr)$
    \item $\op\ \op\ s\longto \op\ \snd\ \bigl((v,f),(u,g)\to(f(v,u),g),\epsilon,s\bigr)$
    \item $\op\ \value\ s\longto \value\ \snd\ \bigl((v,f),u\longto f(v,u),\epsilon,s\bigr)$
\elist
Where $\fst,\snd$ are the first and second projection maps respectively.
Notice that here things of the form $(n,f)$ etc. are {\it values} (which are just arbitrary, and different from \value s).
These abstract types we have just defined allow for basic arithmetic:
$$ \eqalign{
    \value_\infty(1) \op_1(+) \value_\infty(2) \op_2(\cdot) \value_\infty(3)\eend_0 &\longto \op_1(1,+)\value_\infty(2)\op_2(\cdot)\value_\infty(3)\eend_0\cr
    &\longto \op_1(1,+)\op_2(2,\cdot)\value_\infty(3)\eend_0\cr
    &\longto \op_1(1,+)\op_2(2,\cdot)\value_0(3)\cr
    &\longto \op_1(1,+)\value_0(6)\cr
    &\longto \value_0(7)
} $$
Other than numbers, $\value$ can also be strings and lists.

We further define $\lparen$ and $\rparen$, as well as $\vrparen$ (value rparen) as follows:
\blist
    \item $\value\ \rparen\ s\longto \vrparen\ \snd\ \bigl(n\to n,\epsilon,s\bigr)$
    \item $\op\ \vrparen\ s\longto \vrparen\ \snd\ \bigl((f,n),m\to f(n,m),\epsilon,s\bigr)$
    \item $\lparen\ \vrparen\ s\longto \value\ \fst\ \bigl(n\to n,\epsilon,s\bigr)$
\elist

\noindent{\bf Lists:}
We add $\lbrack$, $\rbrack$, and $\list$.
\blist
    \item $\lbrack\ {\astyle\sigma}\ s\longto \lbrack\ \snd\ \bigl(\ell,u\to(\ell,u),\epsilon,s\bigr)$ for ${\astyle\sigma}\neq\lbrack,\rbrack$
    \item $\lbrack\ \rbrack\ s\longto \list\ {\sf infty}\ \bigl(\ell\to\ell,\epsilon,s\bigr)$
\elist
...
\lmultlines{
    \lbrack_0 \value_\infty(1) \op_1(+) \value_\infty(2)\eend_0 \value_\infty(2)\eend_0 \rbrack_0\cr
    &\longto \lbrack_0 \op_1(1,+) \value_\infty(2)\eend_0 \value_\infty(2)\eend_0\rbrack_0\cr
    &\longto \lbrack_0\op_1(1,+)\value_0(2)\value_\infty(2)\eend_0\rbrack_0\cr
    &\longto \lbrack_0\value_0(3)\value_\infty(2)\eend_0\rbrack_0\cr
    &\longto \lbrack_0(3)\value_\infty(2)\eend_0\rbrack_0\cr
    &\longto \lbrack_0(3)\value_0(2)\rbrack_0\cr
    &\longto \lbrack_0(3,2)\rbrack_0\cr
    &\longto \list_\infty(3,2)\cr
}
...
For example
$$ \eqalign{
    \lett_\infty x_\infty \equal_0 \value_\infty(2)\eend_0 & \longto \lett_\infty(x) \equal_0 \value_\infty(2)\eend_0\cr
    &\longto \leteq_0(x) \value_\infty(2)\eend_0\cr
    &\longto \leteq_0(x) \value_0(2)\cr
    &\longto \epsilon\cr
} $$
